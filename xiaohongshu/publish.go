...
// navigateToPublishPage 导航到发布页。
// 若 creator 重定向到登录页，先访问 creator 根域触发 SSO，再重试发布页。
func navigateToPublishPage(pp *rod.Page) error {
	logCookieDiagnostics(pp, "导航 creator 前")
	...
	logrus.Infof("creator 导航后当前 URL: %s", info.URL)
	...
	// creator 返回 401：先访问 creator 根域，触发完整 SSO 流程
	logrus.Warnf("creator 401，通过 creator 根域触发 SSO 重试")
	logCookieDiagnostics(pp, "creator 401 后")

	if err2 := pp.Navigate("https://creator.xiaohongshu.com"); err2 != nil {
		return errors.New("访问 creator 根域失败")
	}
	if err2 := pp.WaitLoad(); err2 != nil {
		logrus.Warnf("creator 根域加载超时（non-fatal）: %v", err2)
	}
	// 等待 SSO 重定向链（JS 驱动，可能经过多次跳转）
	time.Sleep(5 * time.Second)

	if info2, err2 := pp.Info(); err2 == nil {
		logrus.Infof("SSO 后 URL: %s", info2.URL)
		// 仍在登录页，SSO 未完成
		if strings.Contains(info2.URL, "/login") {
			logrus.Warnf("SSO 未完成，session 可能已失效，请重新登录")
			return errors.New("creator 认证失败，请重新登录（delete_cookies 后重新扫码）")
		}
	}

	// SSO 完成，导航到发布页
	if err2 := pp.Navigate(urlOfPublic); err2 != nil {
		return errors.Wrap(err2, "SSO 后导航到发布页失败")
	}
	if err2 := pp.WaitLoad(); err2 != nil {
		logrus.Warnf("SSO 后等待发布页加载出现问题: %v，继续尝试", err2)
	}
	time.Sleep(2 * time.Second)
	if err2 := pp.WaitDOMStable(time.Second, 0.1); err2 != nil {
		logrus.Warnf("SSO 后等待 DOM 稳定出现问题: %v，继续尝试", err2)
	}
	time.Sleep(1 * time.Second)

	if info3, err3 := pp.Info(); err3 == nil {
		logrus.Infof("SSO 后发布页 URL: %s", info3.URL)
	}
	return nil
}
...
func mustClickPublishTab(page *rod.Page, tabname string) error {
	...
	if err != nil {
		if img, serr := page.Screenshot(false, nil); serr == nil {
			...
			logrus.Warnf("发布页未找到 div.upload-content，截图已保存: %s", path)
		}
		info, _ := page.Info()
