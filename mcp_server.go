...
// SwitchAccountArgs 切换账号参数
type SwitchAccountArgs struct {
	Name string `json:"name" jsonschema:"要切换到的账号名"`
}

// AddAccountArgs 添加账号参数
type AddAccountArgs struct {
	Name string `json:"name" jsonschema:"新账号名，不能与已有账号重复"`
}

// RemoveAccountArgs 删除账号参数
type RemoveAccountArgs struct {
	Name string `json:"name" jsonschema:"要删除的账号名，不能是当前激活账号"`
}

// BroadcastPublishArgs 广播发布参数
type BroadcastPublishArgs struct {
	Accounts   []string `json:"accounts" jsonschema:"目标账号名列表，向这些账号同时发布内容"`
	Title      string   `json:"title" jsonschema:"内容标题（小红书限制：最多20个中文字或英文单词）"`
	Content    string   `json:"content" jsonschema:"正文内容，不包含以#开头的标签"`
	Images     []string `json:"images" jsonschema:"图片路径列表（至少1张）。支持 HTTP/HTTPS 链接或本地绝对路径"`
	Tags       []string `json:"tags,omitempty" jsonschema:"话题标签列表（可选），如 [美食, 旅行]"`
	ScheduleAt string   `json:"schedule_at,omitempty" jsonschema:"定时发布时间（可选），ISO8601格式，支持1小时至14天内"`
	IsOriginal bool     `json:"is_original,omitempty" jsonschema:"是否声明原创（可选）"`
	Visibility string   `json:"visibility,omitempty" jsonschema:"可见范围（可选）：公开可见(默认)、仅自己可见、仅互关好友可见"`
}

// CreatorPhoneLoginArgs creator 手机号登录参数
type CreatorPhoneLoginArgs struct {
	Phone string `json:"phone" jsonschema:"中国大陆手机号（11位数字）"`
}

// CreatorVerifyOTPArgs creator 验证码参数
type CreatorVerifyOTPArgs struct {
...
func registerTools(server *mcp.Server, appServer *AppServer) {
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "list_accounts",
			Description: "列出所有已保存的小红书账号及当前激活账号",
			Annotations: &mcp.ToolAnnotations{
				Title:        "List Accounts",
				ReadOnlyHint: true,
			},
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "switch_account",
			Description: "切换当前激活的小红书账号",
			Annotations: &mcp.ToolAnnotations{
				Title: "Switch Account",
			},
		},
		withPanicRecovery("switch_account", func(ctx context.Context, req *mcp.CallToolRequest, args SwitchAccountArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleSwitchAccount(ctx, args.Name)
			return convertToMCPResult(result), nil, nil
		}),
	)
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "add_account",
			Description: "添加新的小红书账号（添加后自动切换为激活账号，需使用 get_login_qrcode 扫码登录）",
			Annotations: &mcp.ToolAnnotations{
				Title: "Add Account",
			},
		},
		withPanicRecovery("add_account", func(ctx context.Context, req *mcp.CallToolRequest, args AddAccountArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleAddAccount(ctx, args.Name)
			return convertToMCPResult(result), nil, nil
		}),
	)
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "remove_account",
			Description: "删除指定的小红书账号及其 cookies（当前激活账号无法删除，需先切换）",
			Annotations: &mcp.ToolAnnotations{
				Title:           "Remove Account",
				DestructiveHint: boolPtr(true),
			},
		},
		withPanicRecovery("remove_account", func(ctx context.Context, req *mcp.CallToolRequest, args RemoveAccountArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleRemoveAccount(ctx, args.Name)
			return convertToMCPResult(result), nil, nil
		}),
	)
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "broadcast_publish",
			Description: "向多个小红书账号广播发布同一图文内容，逐账号顺序发布并汇总结果",
			Annotations: &mcp.ToolAnnotations{
				Title:           "Broadcast Publish",
				DestructiveHint: boolPtr(true),
			},
		},
		withPanicRecovery("broadcast_publish", func(ctx context.Context, req *mcp.CallToolRequest, args BroadcastPublishArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleBroadcastPublish(ctx, args)
			return convertToMCPResult(result), nil, nil
		}),
	)

	// 工具 19: creator 手机号登录（发送验证码）
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "creator_phone_login",
			Description: "向 creator.xiaohongshu.com 发送手机验证码登录请求，返回截图供确认。发送成功后需调用 creator_verify_otp 填写验证码完成登录",
			Annotations: &mcp.ToolAnnotations{
				Title: "Creator Phone Login",
			},
		},
		withPanicRecovery("creator_phone_login", func(ctx context.Context, req *mcp.CallToolRequest, args CreatorPhoneLoginArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleCreatorPhoneLogin(ctx, args.Phone)
			return convertToMCPResult(result), nil, nil
		}),
	)
	...
	mcp.AddTool(server,
		&mcp.Tool{
			...
			Description: "填写验证码完成 creator.xiaohongshu.com 登录，需先调用 creator_phone_login",
			Annotations: &mcp.ToolAnnotations{
				Title:           "Creator Verify OTP",
				DestructiveHint: boolPtr(false),
			},
		},
		withPanicRecovery("creator_verify_otp", func(ctx context.Context, req *mcp.CallToolRequest, args CreatorVerifyOTPArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleCreatorVerifyOTP(ctx, args.OTP)
			return convertToMCPResult(result), nil, nil
		}),
	)
